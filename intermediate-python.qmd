...
Let's say every time you call a method that potentially fails, you need to check if it succeeded before using the result.
...
This can get very messy very quickly, especially if there are ways to recover from the error.

(Unrecoverable errors are easier to handle, you can just let the program crash, optionally with a custom message.)
...
* Avoid catching `Exception`, or omitting the exception type entirely. This will catch all exception, which can lead to hard-to-find bugs.
...
A feature we steer clear of in 121/122 is generators and iterators. These are an incredibly powerful tool for writing efficient code, especially when dealing with large amounts of data.
...
for x in range(10000000):
    print(x)
    if f(x) == 0:  # commonly we don't actually need all of the values
        break


```

If `range` returned a list, it would need to allocate a list of size $10^7$ in memory, which would be very slow and use a lot of memory.

Instead range looks something like this:
...
`yield` is a special keyword that returns a value from the function, but doesn't exit the function.
...
The syntax is that after a variable definition (typically a function parameter) you can add a colon and the type.

You can also add a return type annotation after the closing parenthesis of the function signature with the `-> int` syntax.
...
There are a lot of helper types in the `typing` module:
